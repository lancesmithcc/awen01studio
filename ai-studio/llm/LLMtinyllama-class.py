"""
TinyLlama adapter for AWEN01 Studio.
"""
...
logger = get_logger("llm.tinyllama")


class LLMtinyllama(BaseModelAdapter):
    """
    Adapter for TinyLlama GGUF checkpoints.
    """
    ...
    def __init__(self, model_path: Path, *, device: str | None = None) -> None:
        ...
        self._context_window = 2048
        self._default_max_tokens = int(os.getenv("AWEN_LLM_MAX_TOKENS", "1536"))  # 1536 allows room for prompt in 2048 context
    ...
    def load(self) -> None:
        ...
        if self._force_mock or Llama is None:
            ...
            logger.warning("Using TinyLlama mock mode (%s).", reason)
            self._mark_loaded()
            return
        ...
        logger.info("Loading TinyLlama GGUF from %s (threads=%s, gpu_layers=%s)", self._model_path, self._threads, self._gpu_layers)
        self._llm = Llama(
    ...
    def infer(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        system_prompt = payload.get("system_prompt", "You are AWEN01, a helpful and friendly AI assistant. Keep responses natural, conversational, and concise.")
        temperature = payload.get("temperature", 0.7)
        max_tokens = int(payload.get("max_tokens", self._default_max_tokens))
        logger.debug("TinyLlama prompt len=%s temperature=%s max_tokens=%s", len(prompt), temperature, max_tokens)

        if self._use_mock or not self._llm or not prompt:
            response = f"[TinyLlama mock completion]\n{system_prompt}\n\nUser: {prompt}\nAssistant:"
        ...
        # TinyLlama chat format - simple and clean
        ...
        # Add stop tokens to prevent continuation of conversation format
        stop_tokens = payload.get("stop") or []
        stop_tokens.extend(["User:", "user:", "\nUser:", "\nuser:", "Assistant:", "\nAssistant:"])
        ...
        # Clean up any "user:" prefixes that might have been generated
        lines = text.split("\n")
        cleaned_lines = []
        for line in lines:
        ...
        text = text.rstrip("User:").rstrip("user:").strip()
        
        tokens = len(text.split())
        if not text:
            text = "[TinyLlama completion returned empty output]"
